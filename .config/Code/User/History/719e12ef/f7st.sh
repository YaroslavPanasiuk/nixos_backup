...
if [ "$BATTERY_STATUS" == "Discharging" ] && [ "$BATTERY_LEVEL" -le "$CRITICAL_THRESHOLD" ]; then
    notify-send -a "Low battery" -i "/home/yaros/Downloads/low-battery.png" -u critical "CHARGE NOW (${BATTERY_LEVEL}% left)"
    exit
fi

if [ "$BATTERY_STATUS" == "Discharging" ] && [ "$BATTERY_LEVEL" -le "$THRESHOLD" ]; then
    notify-send -a "Low battery" -i "/home/yaros/Downloads/low-battery(1).png" "Battery level is at ${BATTERY_LEVEL}%. Please plug in your charger"
fi