...
if [ "$1" = "-c" ]; then 
    ...
    scrcpy -d --video-encoder OMX.google.h264.encoder --video-source=camera --camera-facing=back --camera-ar=16:9 --v4l2-sink=/dev/video0 -r file.mp4
    exit
fi
...
if [ -n "$ip" ]; then
    ...
    if [ "$connected" = "connected" ]; then 
        ...
        scrcpy -e --video-encoder OMX.google.h264.encoder --video-source=camera --camera-facing=back --camera-ar=16:9 --v4l2-sink=/dev/video0 -r file.mp4
