...
{ config, pkgs, ... }:
{
  ...
  users.users.yaros = {
    ...
    extraGroups = [ "networkmanager" "wheel" "input" "libvirtd" "kvm" "adbusers"];
    packages = with pkgs; [
    #  thunderbird
    ];
  };
  ...
  systemd.services.battery = {
    serviceConfig = { 
      ExecStart = "/home/yaros/Documents/battery_listener.sh";  # Path to your script or binary
